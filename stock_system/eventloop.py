    ...
    def remove_binding(self, evt, func):
        ...
        with self.etf_lock:
            ...
            try:
                ...
                debug_print("%s doesn't bound for evt(%s)"%(str(func), evt))
...
class EventHandlerNotifier(EventHandler):
    ...
    def observed_in_notifier(self, *args):
        debug_print("[Notifier][observed_in_notifier] : %s"%(args))

    @classmethod
    def callback(cls, *args):
        debug_print("[Notifier][callback] classmethod called")

    def called_by_outside_binding(self, *args):
        debug_print("[Notifier][callback] called_by_outside_binding ... ")
...
class EventHandlerObserver(EventHandler):
    ...
    def observed(self, *args):
        debug_print("[Observer][observed] : %s"%(args))
...
def config_sigint_handler():
    ...
    def signal_handler(signal, frame):
        debug_print('You pressed Ctrl+C!')
        evtloop.shutdown()
        eventloop_started = False
        assert evtloopevtloop.is_shutdown(), "evtloop should be down !"
...
def shutdown_eventloop():
    """
    Should be only called for test
    """
    evtloop = globals()["__evtloop"]
    if evtloop:
        evtloop.shutdown()
...
if __name__ != "__main__":
    debug_print('[eventloop.py] Starting eventloop ! ')
